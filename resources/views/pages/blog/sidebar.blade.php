        ...
        <ul class="collection">
            @foreach($popularposts as $post)
                <li class="collection-item">
                    <a href="{{ route('blog.show',$post->slug) }}" class="pink-text text-darken-4">
        ...
        <ul class="collection">
            @foreach($archives as $stats)
                <li class="collection-item">
                    ...
                    <a href="/blog/?month={{ $stats['month'] }}&year={{ $stats['year'] }}" class="pink-text text-darken-4">

                        {{ $stats['month'] . ' ' . $stats['year'] }}

                        <span class="badge pink darken-1 white-text">{{ $stats['published'] }}</span>
                    </a>
                </li>
            @endforeach
        </ul>
    ...
    <div class="card-content">
        ...
        @foreach($tags as $tag)
            ...
            <a href="{{ route('blog.tags',$tag->slug) }}">
                ...
                <span class="btn-small pink white-text m-b-5 card-no-shadow">{{ $tag->name }}</span>
